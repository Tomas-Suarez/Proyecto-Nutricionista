...
const indicesAcordeon = ref<number[]>([0]); 
...
const inicializarEditor = async () => {
    ...
    try {
        ...
        if (planGrupos.value.length > 0) {
            alimentosStore.buscarDesdeCero(planGrupos.value[indiceGrupoActivo.value].id, '');
        }
    } catch (e) {
        console.error(e);
    ...
    if (listaComidas.length > 0) {
        listaComidas.forEach((dc: any) => {
            ...
            const itemConDetalle = { 
                id_Comida: dc.id_Comida || dc.Id_Comida, 
                nombre: dc.nombreComida || dc.NombreComida, 
                detalle: dc.cantidad || dc.Cantidad,
                nombreCategoria: nombreCatGuardado
            };
            ...
            if (dia === 0) {
                ...
                if (ejemplosMenu.value[indexMenu]) {
                    const momentoVisual = mapEnumToMomentoVisual(dc.momento || dc.Momento);
...
const onDropClasificacion = (evt: DragEvent, esPermitido: boolean) => {
    ...
    if (data && origen === 'catalogo') {
        ...
        const nuevoItem = { 
            ...alimento, 
            detalle: '', 
            nombreCategoria: grupoActual.value.nombre 
        };
    ...
    if (data) {
        ...
        if (menuActual && menuActual.comidas && menuActual.comidas[momento]) {
            menuActual.comidas[momento].push({
                ...alimento,
                nombreCategoria: alimento.nombreCategoria || 'Menú'
            });
        }
    }
};
    ...
    ejemplosMenu.value.forEach((menu, index) => {
        ...
        momentosDia.forEach(momentoVisual => {
            menu.comidas[momentoVisual]?.forEach((item: any) => {
                listaComidas.push({ 
                    id_Comida: item.id_Comida, 
                    nombreCategoria: item.nombreCategoria || 'Menú',
                    es_Permitido: true, 
                    cantidad: item.detalle || '', 
                    dia: diaBackend, 
            ...
            <template #content>
                <div class="d-flex justify-content-between align-items-center">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <h6 class="fw-bold text-primary m-0">CONFIGURACIÓN DEL PLAN</h6>
                    </div>
                    <Button label="Guardar Plan" icon="pi pi-save" @click="guardar" :loading="loading" />
                </div>
                <Divider />
                <div class="section-dark">
                    <div class="row g-3">
                        <div class="col-md-6"><label class="small fw-bold">Nombre del Plan</label><InputText v-model="dietaEnEdicion.nombre" class="w-100" /></div>
                        <div class="col-md-2"><label class="small fw-bold">Fecha Inicio</label><Calendar v-model="dietaEnEdicion.fechaInicio" showIcon dateFormat="dd/mm/yy" class="w-100" /></div>
                        <div class="col-md-2"><label class="small fw-bold">Fecha Fin</label><Calendar v-model="dietaEnEdicion.fechaFin" showIcon dateFormat="dd/mm/yy" class="w-100" /></div>
                        ...
                        <div class="col-12"><label class="small fw-bold">Notas / Diagnóstico</label><InputText v-model="dietaEnEdicion.descripcion" placeholder="..." class="w-100" /></div>
                    </div>
                </div>
            </template>
        </Card>
        ...
        <Card class="shadow-sm border-0">
            <template #content>
                <Accordion :multiple="true" v-model:activeIndex="indicesAcordeon">
                    <AccordionTab header="1. Grupos Alimenticios">
                            ...
                            <div class="section-dark">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-9">
                                        <div class="row h-100">
                                            <div class="col-md-6">
                                                <div class="h-100 border border-success rounded p-2 d-flex flex-column gap-2" style="background: rgba(34, 197, 94, 0.05)">
                                                    <div class="d-flex justify-content-between text-success fw-bold small px-2">
                                                        <span>PERMITIDOS</span>
                                                        <Badge :value="grupoActual.permitidos.length" severity="success"></Badge>
                                                    </div>
                                                    <div class="flex-grow-1 drop-zone" @dragover.prevent @drop="onDropClasificacion($event, true)">
                                                        <ScrollPanel style="height: 360px">
                                                            <div class="d-flex flex-column gap-2 pe-2">
                                                                <div v-for="(item, i) in grupoActual.permitidos" :key="i" class="p-2 rounded bg-surface-card border-start border-4 border-success mb-2 shadow-sm">
                                                                    <div class="d-flex justify-content-between mb-1">
                                                                        <span class="small fw-bold">{{ item.nombre }}</span><i class="pi pi-times text-danger cursor-pointer" @click="eliminarDeClasificacion(true, i)"></i>
                                                                    </div>
                                                                    <InputText v-model="item.detalle" placeholder="Cant. Ej: 1 unidad" class="w-100 p-inputtext-sm" />
                                                                </div>
                                                            </div>
                                                        </ScrollPanel>
                                                    </div>
                                            ...
                                            <div class="col-md-6">
                                                <div class="h-100 border border-danger rounded p-2 d-flex flex-column gap-2" style="background: rgba(239, 68, 68, 0.05)">
                                                    <div class="d-flex justify-content-between text-danger fw-bold small px-2">
                                                        <span>EVITAR / LIMITAR</span>
                                                        <Badge :value="grupoActual.evitar.length" severity="danger"></Badge>
                                                    </div>
                                                    <div class="flex-grow-1 drop-zone" @dragover.prevent @drop="onDropClasificacion($event, false)">
                                                        <ScrollPanel style="height: 360px">
                                                            <div class="d-flex flex-column gap-2 pe-2">
                                                                <div v-for="(item, i) in grupoActual.evitar" :key="i" class="p-2 rounded bg-surface-card border-start border-4 border-danger mb-2 shadow-sm">
                                                                    <div class="d-flex justify-content-between mb-1">
                                                                        <span class="small fw-bold">{{ item.nombre }}</span><i class="pi pi-times text-secondary cursor-pointer" @click="eliminarDeClasificacion(false, i)"></i>
                                                                    </div>
                                                                    <InputText v-model="item.detalle" placeholder="Nota..." class="w-100 p-inputtext-sm" />
                                                                </div>
                                                            </div>
                                                        </ScrollPanel>
                                                    </div>
                        ...
                        <AccordionTab header="2. Ejemplos de Menú">
                            <div class="mb-3 d-flex gap-2 overflow-auto pb-2">
                                <Button v-for="(grupo, index) in planGrupos" :key="grupo.id" :label="grupo.label"
                            ...
                            <div class="section-dark">
                                <div class="row g-3">
                                    <div class="col-md-3">
                                        <div class="d-flex flex-column gap-2 h-100">
                                            <div class="small fw-bold text-success">PERMITIDOS: {{ planGrupos[indiceGrupoMenu]?.label }}</div>
                                            <ScrollPanel style="height: 400px" class="border rounded p-2 bg-black-opacity">
                                                <div class="d-flex flex-column gap-2">
                                                    <div v-for="item in permitidosParaMenu" :key="item.id_Comida"
                                                        ...
                                                        draggable="true" @dragstart="onDragStart($event, item, 'permitidos_menu')">
                                    ...
                                    <div class="col-md-9">
                                        <TabView v-model:activeIndex="menuActivoIndex">
                                            <TabPanel v-for="(menu, idx) in ejemplosMenu" :key="idx" :header="menu.nombre" :value="idx">
                                                <div class="row g-3">
                                                    <div class="col-md-6" v-for="momento in momentosDia" :key="momento">
                                                        <div class="p-3 border rounded bg-black-opacity h-100 drop-zone" :style="getEstiloMomento(momento).border" @dragover.prevent @drop="onDropMenu($event, momento)">
                                                            <div class="d-flex justify-content-between align-items-center mb-2 pb-2 border-bottom">
                                                                <span class="small fw-bold text-uppercase" :style="{ color: getEstiloMomento(momento).color }">{{ momento }}</span>
                                                                <Badge :value="menu.comidas[momento]!.length" severity="info"></Badge>
                                                            </div>
                                                            <div class="d-flex flex-column gap-2 mb-2">
                                                                <div v-for="(comida, cIdx) in menu.comidas[momento]" :key="cIdx" class="d-flex justify-content-between align-items-center p-2 rounded bg-surface-card border shadow-sm">
                                                                    <span class="small fw-bold">{{ comida.nombre }}</span>
                                                                    <i class="pi pi-times text-danger cursor-pointer text-xs" @click="eliminarDelMenu(momento, cIdx)"></i>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
...
<style scoped>
.section-dark { background: rgba(255, 255, 255, 0.03); border-radius: 12px; padding: 1rem; }
.bg-black-opacity { background-color: rgba(0, 0, 0, 0.2); }
.bg-surface-card { background-color: #2d2b45; border: 1px solid #3f3d56; }
.drop-zone { min-height: 150px; transition: background-color 0.2s; }
.drop-zone:hover { background-color: rgba(255, 255, 255, 0.05); }
.item-draggable { transition: transform 0.2s; }
.item-draggable:hover { transform: translateX(5px); border-color: #6366f1 !important; }
